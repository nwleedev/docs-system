...
`dev-docs`는 코딩 에이전트와 함께 일하는 팀이 작업 지침과 개발 문서를 어떻게 관리할지 연구하는 저장소입니다. 애플리케이션도 아니고, 어느 프로젝트에나 그대로 적용할 수 있는 통합 템플릿도 아닙니다.
...
- 사람이 결정한 내용과 AI가 추론한 내용을 구분합니다. 요구사항과 제품 결정은 사람이 명시적으로 확인해야 합니다.
- 한 규칙은 한 문서에서 관리합니다. 같은 규칙을 여러 파일에 복사하지 않고 관리 문서로 연결합니다.
- 린트, 타입 검사, 스키마 검사, 테스트, CI, 실행 결과와 사람의 검토로 확인할 수 있는 규칙은 해당 수단으로 확인합니다. 문서에 적었다는 사실만으로 준수 여부를 판단하지 않습니다.
