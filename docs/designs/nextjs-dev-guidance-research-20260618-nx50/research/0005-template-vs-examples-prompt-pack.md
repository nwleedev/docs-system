...
템플릿 강화만으로 구현할 수는 있지만, 사용자가 범위가 넓은 한 문장만 입력한 경우 기술 스택을 세부 개발 지침 대상으로 나누고 권장 사례와 피해야 할 사례까지 문서화하도록 안정적으로 유도하기에는 부족하다. 이 입력 파일은 작업 예시에만 사용되며 다른 저장소에 적용하는 기본 전제가 아니다.
