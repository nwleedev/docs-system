...
이 계획은 [`requirements.md`](./requirements.md)의 "목표", "유지할 전역 원칙"과 "완료 조건"을 기준으로 사용자가 수정할 수 있는 Skill 참고 구현을 제공한다. 자동 설치기, Plugin 패키지와 별도의 Git workflow Skill은 추가하지 않는다.
...
- 두 `src` 예시의 Git 규칙이 별도의 Git Skill을 요구하지 않는다고 기록한다.
...
**검증:** 두 `src` 예시만으로 요청된 worktree와 commit 작업의 조건을 확인할 수 있다.
