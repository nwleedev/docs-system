...
| 가운뎃점·이모지 표준과 접근성 지침 | 가운뎃점은 한국어 문장부호이고 이모지는 Unicode 텍스트 표현이다. 이모지는 문맥과 대체 설명을 확인해야 한다. | 해당 문자의 빈도로 AI 작성 여부를 판정한 한국어 품질 연구는 아니다. |
...
기존 네 차례에 가운뎃점과 이모지를 다룬 세 차례를 더해 모두 일곱 차례 검색과 원문 대조를 수행했다.
...
마지막 검색에서는 가운뎃점이나 이모지를 신뢰할 수 있는 AI 작성 표지로 삼는 1차 근거가 나오지 않았다. 기존의 `대상 글 표본`, `다차원 판정`, `자동 후보 검사`, `사람의 블라인드 비교`에 `채널·문맥·접근성 판정`을 더하는 범주에서 결과가 반복됐다. 인앱 브라우저로 ACL 논문, LREAD v3, 국립국어원·Unicode·W3C 자료를 다시 확인했으며, LREAD의 초기 초록보다 제한 조건이 명확한 2026년 3월 17일 개정본을 기준으로 기록했다. 이 조사 범위는 포화 상태로 판단한다.
