...
문장 사이의 관계는 새 린터나 별도 검토 스킬이 아니라 현재 `skills/use-words-review/SKILL.md`의 입력 범위와 판정 기준에서 다뤄야 한다. 검토자가 변경된 문장만 받으면 앞뒤 관계를 판단할 수 없으므로 필요한 문단이나 절을 함께 전달하고, 관계가 끊긴 경우에는 연결되지 않는 두 위치를 모두 보고해야 한다.

`skills/use-words-review/references/examples.md`에는 역할 이름이 설명 없이 달라지는 경우, 앞에서 정하지 않은 결과를 참조하는 경우, 지칭 대상이 둘 이상인 경우, 제안을 승인된 결정처럼 이어 쓰는 경우와 정상 대조문을 추가해야 한다. 루트와 배포용 AGENTS 문서에는 문장을 고친 뒤 문단과 문서 전체를 다시 읽는 원칙만 두고 상세 판정 목록은 스킬이 소유해야 한다.
