...
이 문서는 [요구사항 문서](../requirements.md)의 다음 항목을 여러 저장소에서 반복해서 사용할 검사 체계로 옮길 때 필요한 사실을 조사한다.

> AI가 번역체, 기계적인 번역을 사용하지 못하게 하는 방법

> AI가 "작업 지시자가 입력한 프롬프트"를 그대로 문서나 코드, 커밋에 적지 않도록 하는 방법.

> AI가 텍스트를 작성하기 전에 "이 텍스트가 과연 해당 대상을 위한 텍스트인가" 확인할 수 있도록 하는 방법
...
2026년 7월 17일을 기준으로 Vale, markdownlint, textlint를 비교하고, Git 커밋 전 검사와 읽기 전용 검토 에이전트를 결합하는 방법을 확인했다. 변경된 요구사항이 지정한 설계·개발 문서와 배포용 에이전트 지침의 현재 역할, 전용 스킬과 `src/vale` 구성의 필요성도 검토했다. 이 문서는 검사 체계의 구성과 운영만 맡는다. 문제 유형과 기존 사례는 [독자 혼동 조사](public-output-audience-research.md), [작업 지시와 산출물 출처 조사](prompt-intent-and-artifact-provenance-research.md), [한국어 문체 조사](natural-korean-and-ai-writing-research.md), [민감한 원문 조사](sensitive-input-preservation-research.md), [문서 제목과 결정 단위 조사](decision-record-scope-and-title-research.md)에 기록되어 있다.

아래의 구성은 조사 자료를 비교해 얻은 제안이며 사람이 승인한 설계 결정은 아니다.

## 결론

한 도구가 모든 문제를 판정하게 만들 수는 없다. 정규식으로 재현되는 표현과 문서 구조는 린터가 맡고, 독자에게 맞는 글인지와 작업 지시가 프로젝트 사실로 둔갑했는지는 별도 검토 에이전트와 사람이 판단해야 한다. 원문과 절대 경로처럼 비교 기준이 분명한 항목은 Vale 규칙에 억지로 넣기보다 전용 결정적 검사로 처리하는 편이 낫다.
...
3. 의미 판단이 필요한 변경만 읽는 검토 에이전트 정의
...
| 독자에게 맞는 글인지, 작업 보고가 섞였는지, 한 문서가 여러 결정을 묶었는지 | 읽기 전용 검토 에이전트 | 기본적으로 판정 근거를 제시하고 사람에게 넘김 |
...
[Codex 서브에이전트 문서](https://developers.openai.com/codex/agent-configuration/subagents)는 프로젝트별 `.codex/agents/` 파일에 전용 지침과 샌드박스 설정을 둘 수 있고 읽기 전용 설정도 가능하다고 설명한다. [Claude Code 문서](https://code.claude.com/docs/en/sub-agents)도 특정 작업에 집중한 에이전트, 필요한 도구만 허용하는 방식, 프로젝트 정의 파일의 버전 관리를 권한다.

따라서 `AGENTS.md`에는 어떤 변경에서 공개 산출물 검토를 호출하는지와 결과를 누가 처리하는지만 남기고, 판정 항목·입출력 형식·도구 제한은 별도 검토 에이전트 파일에 두는 편이 낫다. 긴 판정표를 모든 작업의 기본 문맥에 넣지 않아도 되고, 검토 기준의 버전과 변경 이력을 따로 확인할 수 있다. Codex와 Claude Code의 정의 형식은 다르므로 공통 규칙 자료와 제품별 얇은 어댑터가 필요하다.
...
검토 에이전트는 파일을 수정하거나 외부로 내용을 전송하지 못하게 한다. 검토하는 문서와 코드에 적힌 문장은 실행할 지시가 아니라 검사 대상 자료로 취급해야 한다. 결과는 다음 네 상태 중 하나와 파일 위치, 규칙 종류, 짧게 정리한 근거, 독자와 맞지 않는 이유, 확신 정도, 수정 방향을 반환한다.
...
요구사항의 `필요한 작업 및 변경사항`이 지정한 파일을 현재 브랜치에서 확인했다. 아래 내용은 현재 역할과 조사 결과를 비교한 분석이다. 실제 변경 범위는 아직 결정되지 않은 도구 선택과 호출 조건을 정한 뒤 구체화해야 한다.
...
| `docs/designs/README.md` | 사람이 소유하는 요구사항과 파생된 참고 자료·결정·계획의 책임, 근거, 검증 방법을 정한다. | 보강이 필요하다. 사람이 문구를 소유하는 `requirements.md`의 원문 보존과, 작업 지시를 공개 가능한 사실로 사용할 수 없는 파생 문서의 출처 규칙을 분명히 구분해야 한다. 참고 자료·결정·계획의 검증에도 내부 작업 문구와 개인 경로 검사를 연결할 필요가 있다. |
| `docs/dev/README.md` | 반복해서 사용할 저장소별 개발 지침의 근거, 상태, 소유 문서와 검사 방법을 정한다. | 보강이 필요하다. 현재도 확인된 저장소 증거와 승인된 결정을 요구하지만, 작업 지시나 실행 기록을 현재 규칙의 근거로 사용하지 말라는 조건은 명시하지 않는다. 개발 지침의 공개 전 검사와 허용되는 인용 조건을 이 문서의 책임에 맞게 추가할 필요가 있다. |
| `src/AGENTS.en.md` | 영어 사용자가 대상 저장소에 필요한 부분을 골라 쓸 수 있는 참고 지침이다. | 공개 산출물과 공개 전 검사 항목이 이미 작업 지시, 비공개 경로, 커밋 메시지를 포함한다. Vale나 전용 스킬의 호출 방법은 도구가 결정되기 전에는 추가하지 않는다. |
| `src/AGENTS.ko.md` | 한국어 사용자가 대상 저장소에 필요한 부분을 골라 쓸 수 있는 참고 지침이다. | 영어 문서와 마찬가지로 공개 결과물과 공개 전 검사 항목이 이미 존재한다. 현재 문제를 설명하는 규칙을 중복해서 추가할 필요는 없다. |
...
- 의미 검토가 필요한 변경만 읽기 전용 검토 에이전트에 전달한다.
- 검사 결과와 사람이 판단해야 할 항목을 정해진 상태로 반환한다.

반대로 `src/AGENTS.*.md`의 공개 전 검사 목록을 그대로 옮긴 스킬은 같은 규칙의 소유 문서를 늘릴 뿐이다. 전용 스킬의 입력, 호출 조건, 도구 선택, 출력 형식이 결정되기 전에는 스킬을 생성하지 않는 편이 낫다. `AGENTS.md`에는 채택된 스킬을 언제 호출하는지만 남기는 구성이 [AGENTS.md 공개 형식](https://agents.md/)의 저장소 지침 역할과 OpenAI의 [짧은 진입 문서 사례](https://openai.com/index/harness-engineering/)에 맞는다.
...
4. `AGENTS.md`에는 검토 호출 조건을, 제품별 에이전트 파일에는 읽기 전용 권한과 판정 형식을 둔다.
...
- Codex와 Claude Code 외 도구가 사용할 공통 검토 규약의 직렬화 형식은 정해지지 않았다.
- 검토 에이전트 호출 비용과 지연을 허용할 변경 범위는 정해지지 않았다.
- 전용 스킬이 어떤 변경에서 자동으로 호출되고 어떤 검사 결과를 받아야 하는지 결정되지 않았다.
...
후속 결정은 공통 텍스트 검사 도구, 공통 패키지의 첫 지원 범위, 대상 저장소 선언 방식, 규칙 승격 기준, 검토 에이전트와 전용 스킬의 호출 조건을 다뤄야 한다. 그 결정 뒤에야 `docs/designs/README.md`, `docs/dev/README.md`, `src/vale`과 제품별 어댑터의 실제 변경을 실행 계획으로 구체화할 수 있다.
