...
이 문서는 [요구사항 문서](../requirements.md)의 `요구사항`, `작업 지시 문구가 산출물의 내용과 구조를 대신하면 안 되는 경우`, `필요한 작업 및 변경사항`, `리뷰 에이전트`, `자연스러운 문장 연결`, `토큰 비용`, `스킬에 README.md 파일 제거`, `공통 검토 절차와 한국어 판정 기준 분리`, `주의`에 해당하는 문제를 여러 저장소에서 반복해서 검사하는 데 필요한 사실을 조사한다. 번역체와 기계적인 표현, 작업 지시 원문의 혼입, 독자 부적합, 개인 경로와 비공개 식별자를 검사 대상으로 삼고, 검토 스킬의 실행 비용과 배포 방법을 확인한다.

첫 조사는 2026년 7월 17일에 수행했다. Vale, markdownlint, textlint를 비교하고 Git 커밋 전 검사와 범용 서브에이전트 검토를 결합하는 방법을 확인했다. 7월 28일에는 Codex와 Claude Code의 현재 스킬 및 서브에이전트 문서, Agent Skills 명세, 실제 스킬 저장소와 주석으로 범위를 표시한 배포 사례를 다시 확인했다. 같은 날 언어별 참조 파일을 추가하는 조건을 확인하기 위해 스킬의 점진적 로딩 방식, 상시 프로젝트 지침과 작업별 스킬의 차이, 언어 중립 자료와 언어별 자료를 나누는 표준, 다국어 문서의 용어 관리 지침을 추가로 대조했다. 변경된 요구사항이 지정한 설계 문서, 개발 지침, 배포용 에이전트 지침의 현재 역할도 비교했다. 이 문서는 검사 체계의 구성과 운영만 맡는다. 문제 유형과 기존 사례는 [독자 혼동 조사](public-output-audience-research.md), [작업 지시와 산출물 출처 조사](prompt-intent-and-artifact-provenance-research.md), [한국어 문체 조사](natural-korean-and-ai-writing-research.md), [민감한 원문 조사](sensitive-input-preservation-research.md), [문서 제목과 결정 단위 조사](decision-record-scope-and-title-research.md)에 기록되어 있다.
...
스킬의 공통 실행 절차와 문장 의미 검사는 `SKILL.md`에 남기고, 한국어에서만 필요한 표현 후보와 판정 사례는 `references/korean.md`로 분리하는 편이 맞다. `src/AGENTS.ko.md`는 작성 중에도 필요한 짧은 예방 원칙을 맡고, 상세한 한국어 대응표를 함께 소유하지 않는다. 이 구성은 한국어가 없는 검토에서 한국어 자료를 읽지 않으면서도 모든 언어에서 문장 자체의 뜻과 문장 사이의 관계를 검사한다.

## 검사 책임을 나누는 기준

- **금지된 제어 문자, 개인 계정이 드러나는 절대 경로, 알려진 비밀 값 형식.** 결정적 검사를 먼저 사용한다. 패턴의 의미와 예외가 명확할 때 자동으로 차단할 수 있다.
- **같은 작업 중 제공된 원문과 긴 문자열이 그대로 겹치는지.** 원문을 보관하지 않는 일시 비교를 먼저 사용한다. 정확히 일치하고 보존 대상이 아닐 때 자동 차단 후보로 삼는다.
- **반복해서 발견된 기계적 표현, 잘못된 용어, 문장부호.** Vale를 먼저 사용한다. 오탐이 낮아진 규칙만 자동으로 차단한다.
- **제목 단계, 줄 길이, 잘못된 Markdown 구조.** markdownlint를 선택해 적용한다. 구조 규칙이 합의된 저장소에서 자동으로 차단한다.
- **특정 파일 형식의 구문 트리나 기존 textlint 규칙 활용.** textlint를 선택해 적용한다. 해당 규칙의 오탐을 검증한 뒤 자동으로 차단한다.
- **독자에게 맞는 글인지, 작업 보고가 섞였는지, 한 문서가 여러 결정을 묶었는지.** 범용 서브에이전트를 실행하는 검토 스킬을 먼저 사용한다. 기본적으로 판정 근거를 제시하고 사람에게 넘긴다.
- **다른 사람에게 보여도 되는 인용인지, 문구의 제품 의미가 맞는지.** 사람이 검토하고 최종 결정한다.
...
2026년 7월 28일에 `references/examples.md`와 `references/korean.md`의 판정이 정해진 사례 11개를 같은 순서와 기준으로 비교했다. 필요한 정보가 빠진 문장 두 개, 문장 사이에서 역할·승인 범위·지칭 대상·결정 상태가 끊긴 사례 네 개, 뜻을 정할 근거가 없는 `접근 경로`와 `공개 저장소` 사례, 길지만 관계가 분명한 문장, 서로 연결된 문장과 정확한 파일 경로 용례를 포함했다.

첫 비교에서는 공통 기준이 같은 역할 이름의 책임 유지에만 초점을 맞춰, 이웃한 문장에서 역할 이름이 바뀌는 사례의 관계를 충분히 지시하지 못했다. 낮은 설정은 `needs human input` 다섯 건, 중간 설정은 여섯 건, 높은 설정은 세 건을 다른 상태로 판정했다. 이 가운데 낮음과 중간 설정은 두 사례를 `pass`로 잘못 판정했고, 높은 설정은 한 사례를 `pass`로 잘못 판정했다. 모델을 바꾸는 대신 `SKILL.md`에 역할 이름이 바뀌면 두 역할의 관계를 확인하고, 저장소 근거로 빠진 정보를 정할 수 없으면 `needs human input`을 사용한다는 공통 기준을 보완했다.

수정된 기준으로 다시 비교하자 `gpt-5.6-terra`의 낮음과 중간 추론 강도, `gpt-5.6-sol`의 높은 추론 강도 요청이 모두 11개 사례의 판정과 일치했다. 영어만 있는 별도 사례에서도 공통 문장 의미 검사는 모호한 지칭 대상을 찾아냈고 한국어 참조 파일은 읽지 않았다. 한국어 사례에서는 문제 용례 두 개를 `needs human input`, 뜻과 조건이 분명한 정상 용례를 `pass`로 판정했다. 이 결과는 기준에서 빠진 내용을 찾기 위한 탐색 실행이며 낮은 설정을 채택해도 된다는 승인을 뜻하지 않는다.

실행 환경은 각 호출의 입력·출력·전체 토큰과 경과 시간, 실제 제공된 모델 식별자를 반환하지 않았다. 재현 가능한 호출 기록과 담당 검수자의 승인도 저장소에 없다. 따라서 이번 결과는 판정 기준을 수정한 근거로만 사용하고, 모델별 비용이나 응답 시간 비교 또는 낮은 설정 채택의 근거로 사용하지 않는다.

### 설치 환경 확인

2026년 7월 28일에 버전 관리에서 제외되는 임시 프로젝트를 만들고, 배포 안내에 적힌 프로젝트별 경로에 현재 스킬을 설치했다. Codex CLI `0.141.0`은 `AGENTS.md`의 확인 문구와 `.agents/skills/use-words-review/SKILL.md`를 함께 읽었다. 영어 문장을 검토할 때에는 `references/examples.md`만 열고 범용 서브에이전트 한 명에게 검토를 맡겼으며, 지칭 대상과 승인 근거가 빠진 문장을 `needs human input`으로 판정했다.

Codex의 첫 실행은 사용자 설정에 있던 `agents` 값이 현재 CLI가 요구하는 형식과 맞지 않아 시작 전에 실패했다. 사용자 설정을 제외하는 공식 옵션으로 다시 실행하자 프로젝트 지침과 스킬 탐색은 정상 작동했다. 이 결과는 스킬 배포 경로를 확인하지만, 기존 사용자 설정이 새 CLI와 호환된다는 증거는 아니다.

Claude Code `2.1.218`은 `CLAUDE.md`의 확인 문구와 `.claude/skills/use-words-review/SKILL.md`를 함께 읽었다. 읽기 도구만 허용해 서브에이전트를 호출할 수 없게 한 실행에서는 결과 첫머리에 독립 검토를 수행하지 못했다는 경고를 남기고 주 대화에서 검토를 계속했다. 이 실행도 영어 공통 예시만 읽었으며 같은 문장을 `needs human input`으로 판정했다. 따라서 서브에이전트를 제공하지 않는 실제 실행에서 경고와 주 대화 검토를 함께 남기는 대체 절차를 확인했다.

Codex CLI는 호출별 `--model` 옵션을 제공하고 엄격한 설정 검사에서 `model_reasoning_effort=low`를 허용했다. Claude Code는 호출별 `--model`과 `--effort` 옵션을 제공한다. 이 확인은 호출마다 설정을 전달할 수 있다는 뜻이며, 요청한 모델이 실제로 제공됐는지 또는 두 도구의 추론 강도가 같은 계산량을 뜻하는지는 입증하지 않는다.
...
- **현재 역할.** `docs/designs/README.md`를 설계 문서의 단일 기준으로 사용하고 Review, Research, Record, Plan, Validate 순서를 조정한다.
- **변경 필요성.** 지금 바로 고칠 근거는 부족하다. 독자에게 전달할 글과 문구의 출처 규칙은 이 스킬에 복사하지 말고 먼저 `docs/designs/README.md`가 소유해야 한다. README가 새 검사를 정의한 뒤에도 스킬의 기존 Validate와 Finish 절차로 호출되지 않는 검사가 있을 때만 어댑터를 바꾼다.

### `skills/use-dev-guidance/SKILL.md`

- **현재 역할.** `docs/dev/README.md`를 기준으로 저장소 증거, 외부 조사, 검사 선택과 개발 지침 작업을 연결한다.
- **변경 필요성.** 지금 바로 고칠 근거는 부족하다. 개발 지침에서 작업 지시를 근거로 쓰지 않는 규칙은 `docs/dev/README.md`가 먼저 소유해야 한다. 스킬에는 그 규칙을 되풀이하지 않는다.

### `docs/designs/README.md`

- **현재 역할.** 사람이 소유하는 요구사항과 파생된 참고 자료, 결정, 계획의 책임, 근거, 검증 방법을 정한다.
- **변경 필요성.** 원문 보존 예외와 파생 문서의 출처 및 독자 검사를 포함한다. 후속 스킬은 이 규칙을 복사하지 말고 현재 저장소의 README를 검사 입력으로 사용해야 한다.
...
공식 스킬 자료는 `SKILL.md`를 실행 진입 파일로 사용하고 스크립트, 참고 자료와 자산을 선택 사항으로 둔다. 스킬 폴더의 README를 필수 파일이나 자동 탐색 입력으로 지정하지 않는다.

- [Agent Skills 명세](https://agentskills.io/specification)는 `SKILL.md`를 필수 파일로, `references/`를 필요할 때 읽는 선택형 자료로 정의한다.
- [OpenAI 스킬 문서](https://developers.openai.com/codex/skills)는 이름과 설명을 먼저 확인하고 선택된 뒤에 `SKILL.md` 전체를 불러오는 방식을 설명한다.
- [Claude Code 스킬 문서](https://code.claude.com/docs/en/agent-sdk/skills)는 `SKILL.md`와 선택형 스크립트 및 참조 자료로 구성된 스킬 디렉터리를 설명한다.

따라서 `skills/use-words-review/README.md`를 지워도 `SKILL.md`와 필요한 참조 파일만으로 스킬 실행 형식은 유지된다.

글과 문구 검사는 설계 문서, 개발 지침, 코드, UI 문구와 커밋 메시지를 함께 다루므로 기존 두 문서 어댑터 중 하나에 넣으면 적용 대상이 어긋난다. 실행 절차와 공통 의미 판정은 `skills/use-words-review/SKILL.md`, 언어와 관계없는 판정 예시는 `references/examples.md`, 한국어 판정 자료는 `references/korean.md`, 다른 저장소에 적용할 호출 지침은 `src/AGENTS.en.md`와 `src/AGENTS.ko.md`, 설치와 제거 방법은 루트 `README.md`와 `README.ko.md`가 맡는 구성이 각 문서의 현재 독자와 맞는다.
...
Superpowers 리비전 `3dcbd5c`의 `skills/` 트리는 각 스킬의 `SKILL.md`와 필요한 참고 자료를 실행 단위로 관리하며 스킬마다 README를 두지 않는다. terraform-docs는 생성한 README 내용을 시작과 끝 HTML 주석 사이에 넣어 해당 부분만 바꾼다. 이 사례들은 참고 자료를 Skill 디렉터리에 함께 두는 방식과 HTML 주석으로 관리할 부분을 표시하는 방식이 서로 독립적으로 사용될 수 있음을 보여준다. 다만 주석 이름과 교체 동작은 Agent Skills 표준이 아니라 각 배포 도구가 정한 규칙이다. [Superpowers skills 트리](https://github.com/obra/superpowers/tree/3dcbd5c4b48e02263fbf4a3c01e3fe4f81d584d9/skills), [terraform-docs 출력 설정](https://github.com/terraform-docs/terraform-docs/blob/10965cddfa169679511f176cfe67dd0189dc935f/docs/user-guide/configuration/output.md)
...
AGENTS 지침은 모든 작업에서 읽히지만 스킬 본문과 참조 파일은 스킬이 호출된 뒤에만 읽힌다. 작성 단계에서도 필요한 독자·근거·자연스러운 문장 원칙은 AGENTS에 남겨야 한다. 추가 조사에서는 긴 표현 대응표와 실패 및 허용 예시를 한국어 검토에서만 읽는 `references/korean.md`로 옮길 근거를 확인했다. 개별 항목을 옮길 때에는 예방 원칙까지 빠져 스킬 호출 전의 작성 품질이 낮아지지 않는지 따로 확인해야 한다.

## 공통 검토와 한국어 판정 자료를 분리한다

현재 `skills/use-words-review/SKILL.md`는 모든 검토에서 실행되는 본문에 한국어 표현 후보를 직접 나열한다. 또한 문장 자체의 뜻과 문장 사이의 관계처럼 모든 언어에 필요한 검사를 `Korean wording` 판정 항목 안에 둔다. 한국어가 없는 문서에서 이 항목을 적용하지 않으면 공통 의미 검사까지 빠질 수 있고, 적용하면 영어 문서를 검토하면서 필요하지 않은 한국어 목록을 함께 전달하게 된다.

스킬 형식에 관한 공식 자료는 공통 절차를 짧은 진입 파일에 두고 상세 자료를 직접 연결된 참조 파일로 나누는 방향에서 일치한다.

- **필요한 상세 자료만 불러오는 구조가 명세에 포함된다.**
  - [Agent Skills 명세](https://agentskills.io/specification)는 `SKILL.md` 전체가 스킬 선택 뒤에 로드되며, 집중된 참조 파일은 필요할 때 읽고 `SKILL.md`에서 한 단계로 연결하라고 설명한다.
  - [OpenAI 스킬 문서](https://learn.chatgpt.com/docs/build-skills)는 이름과 설명을 먼저 확인한 뒤 선택된 스킬의 `SKILL.md`를 읽는 점진적 로딩 방식과 선택형 `references/` 디렉터리를 설명한다. 설명은 자동 호출 범위를 정하므로 한국어 검토만 하는 스킬처럼 보이지 않게 공통 검토와 한국어 적용 조건을 함께 밝혀야 한다.
  - [Claude Code 스킬 문서](https://code.claude.com/docs/en/skills)는 스킬 본문이 실행 뒤에도 문맥에 남으므로 핵심 절차만 두고 상세한 스타일 가이드와 예시를 보조 파일로 옮기라고 안내한다.
- **상시 지침과 특정 작업에서만 필요한 절차는 역할이 다르다.**
  - [Claude Code 프로젝트 지침](https://code.claude.com/docs/en/memory)은 모든 작업에서 알아야 하는 규칙은 프로젝트 지침에 남기고, 여러 단계의 절차나 일부 작업에서만 필요한 내용은 스킬이나 범위가 정해진 규칙으로 옮기라고 설명한다.
- **공통 자료와 언어별 차이를 분리하면 중복과 관리 오류를 줄일 수 있다.**
  - [Unicode LDML](https://unicode.org/reports/tr35/)은 언어와 지역에 중립적인 자료를 공통 계층에 두고 언어별 자료에는 공통 자료와 다른 값만 두는 상속 구조가 자료 묶음을 작게 만들고 유지보수 오류를 줄인다고 설명한다. 이 표준은 에이전트 스킬 형식을 정하지 않지만, 공통 판정과 한국어 차이를 분리하는 구조를 검토할 때 참고할 수 있는 인접 근거다.
- **언어별 판정은 단순한 번역보다 해당 언어의 문맥과 용어 일관성을 확인해야 한다.**
  - [Google의 다국어 문서 지침](https://developers.google.com/style/translation)은 현지화를 번역과 구분하고, 모호하지 않은 문장, 분명한 지칭 대상과 일관된 용어가 필요하다고 설명한다.
  - [Microsoft의 전 세계 독자를 위한 작성 지침](https://learn.microsoft.com/en-us/style-guide/global-communications/writing-tips)은 단순한 문장 구조, 행위자를 드러내는 표현과 한 개념에 같은 용어를 쓰는 방식을 권고한다.

이 자료를 현재 파일에 적용하면 다음과 같이 책임을 나눌 수 있다.

- `SKILL.md`는 검토 범위, 근거 수집, 결정적 검사, 문맥 선택, 모델 선택, 위임, 결과 조정과 보고 절차를 소유한다. 대상 언어와 관계없이 문장마다 지칭 대상, 행동 주체, 행동, 조건과 결과를 찾고 문장 사이의 관계를 확인한다.
- `SKILL.md`의 설명은 모든 언어의 공개 문구와 문장 의미를 검토한다는 범위를 먼저 밝히고, 한국어가 있을 때 한국어 자연스러움도 검사한다고 덧붙인다. 한국어 표현 목록은 설명에 넣지 않는다.
- `references/korean.md`는 한국어 직역, 번역체, 혼합 표기, 문맥에 따라 뜻이 달라지는 후보 표현과 한국어 문제 및 정상 사례를 소유한다. 한국어가 포함된 결과물이나 한국어 표현의 판정이 필요한 경우에만 `SKILL.md`가 직접 읽도록 한다.
- `references/examples.md`는 작업 보고의 혼입, 근거 없는 주장, 원문 보존, 개인 경로, 문장 의미와 연결처럼 언어와 관계없이 적용되는 검토 사례를 소유한다. 한국어 표현 자체가 판정 이유인 사례는 남기지 않는다.
- `src/AGENTS.ko.md`는 글을 쓰는 동안 항상 필요한 독자, 구체적인 대상과 행동, 자연스러운 한국어, 문장 뜻과 연결에 관한 짧은 원칙을 소유한다. 개별 영어 용어의 한국어 대응표와 판정별 사례는 소유하지 않는다.
- `src/AGENTS.en.md`는 영어 작성 원칙을 계속 소유한다. 영어에서 반복되는 표현 문제가 별도로 확인되기 전에는 `references/english.md`를 만들지 않는다.

`references/korean.md`를 추가해도 참조 파일이 연쇄적으로 서로를 불러오게 만들 필요는 없다. `SKILL.md`에서 두 참조 파일을 각각 한 단계로 연결하고 읽을 조건을 함께 적으면 Agent Skills 명세의 참조 방식과 맞는다. 한국어와 영어를 별도 스킬로 나누면 근거 수집, 모델 선택과 결과 형식이 중복되고, 원본 조각에서 여러 파일을 생성하면 현재 요구사항에 없는 생성기와 동기화 검사가 생긴다. 반복 실패가 확인되지 않은 다른 언어의 참조 파일도 미리 만들 이유가 없다.

새 파일의 내용은 기존 두 파일에서 그대로 복사하지 않는다. 먼저 각 항목이 작성 중 예방 원칙인지, 모든 언어에 적용되는 검토인지, 한국어 표현을 판정하기 위한 자료인지 분류한다. 같은 원칙이 여러 실행 시점에 필요하면 AGENTS에는 짧은 행동 지침만 남기고 `references/korean.md`에는 판정 조건과 대조 사례를 둔다. 두 파일이 같은 단어 목록과 대체 표현을 함께 소유하지 않게 해야 이후 변경이 서로 어긋나지 않는다.
...
- 대표 사례 탐색 결과를 재현할 호출 기록과 담당 검수자의 승인이 없어 낮은 설정의 채택 여부는 정해지지 않았다.
...
이번 조사에 따라 `requirements.md`에는 공통 의미 검사와 한국어 판정 기준의 적용 범위, 새 `references/korean.md`의 역할, `src/AGENTS.ko.md`와 `references/examples.md`의 책임을 추가해야 한다. 하나의 만능 린터를 요구사항으로 추가하지 않고, 기계적으로 판정할 수 있는 문제와 의미 판단이 필요한 문제를 나누는 기존 설계는 유지한다.

후속 작업은 `skills/use-words-review/SKILL.md`에서 같은 공유 단위의 변경을 한 번에 검토하도록 호출 시점과 입력 범위를 유지하면서, 공통 문장 의미 검사를 한국어 자연스러움 검사와 분리해야 한다. 새 `references/korean.md`에는 한국어 표현 후보, 판정 조건과 대조 사례를 옮기고, `references/examples.md`에는 언어와 관계없는 관계 단절 사례와 정상 사례를 남긴다.
...
마지막 대조에서는 OpenAI, Claude Code와 Agent Skills 명세가 공통 절차와 필요할 때 읽는 보조 자료를 나누는 같은 구조를 제시했다. Unicode의 언어 자료 상속 구조와 Google 및 Microsoft의 다국어 작성 지침을 추가로 확인해도 공통 판정을 한국어 자료에 종속시키거나 한국어 표현 목록을 모든 언어에 적용해야 한다는 근거는 나오지 않았다. `references/korean.md` 외에 별도 언어 스킬, 영어 참조 파일, 생성기나 새 검사 계층을 추가해야 할 새로운 근거도 나오지 않아 이번 조사 반복을 중단했다.
...
- 저장소 자료: [`README.md`](../../../../README.md), [`README.ko.md`](../../../../README.ko.md), [`skills/use-words-review/SKILL.md`](../../../../skills/use-words-review/SKILL.md), [`skills/use-design-docs/SKILL.md`](../../../../skills/use-design-docs/SKILL.md), [`skills/use-dev-guidance/SKILL.md`](../../../../skills/use-dev-guidance/SKILL.md), [`docs/designs/README.md`](../../README.md), [`docs/dev/README.md`](../../../dev/README.md), [`src/AGENTS.en.md`](../../../../src/AGENTS.en.md), [`src/AGENTS.ko.md`](../../../../src/AGENTS.ko.md). 2026년 7월 28일 확인.
