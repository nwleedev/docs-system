...
description: Review changed public or tracked text with an independent general-purpose subagent for audience fit, publishable provenance, prompt or work-note leakage, private paths and identifiers, unnatural Korean, and unnecessary symbols. Use after creating or editing documentation, README files, UI or accessibility text, code comments, commit or pull-request text, issue text, or release notes; before committing or sharing those outputs; and whenever the user requests a wording or public-output review.
---
...
1. Identify every changed output that will be committed, published, or shared. Include the candidate commit, pull-request, issue, or release text when applicable.
2. Treat tracked repository text as public unless an applicable repository rule classifies it otherwise.
3. Record the intended readers, what each reader must understand or do, and whether exact source text is part of the artifact's purpose.
...
1. The title, structure, and wording serve the intended readers instead of reporting work to the person who assigned it.
...
4. Claims use publishable evidence, approved decisions, or approved wording; unresolved support, ownership, compatibility, security, privacy, and licensing claims are not invented.
...
- `needs revision`: a specific defect can be corrected without a new human decision;
- `needs human input`: publication depends on unresolved intent, approval, ownership, or policy;
...
List the reviewed outputs, their intended readers, the checks performed, and the per-criterion results. Separate defects that can be revised from questions that require human input. State any missing independent review, unavailable check, redaction, or incomplete evidence.
