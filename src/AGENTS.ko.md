...
- 한국어 공개 문구, 이름 또는 사용자에게 보낼 답변을 작성하거나 고치는 작업을 시작할 때 설치된 `use-words-review` 스킬의 `references/korean.md`를 읽는다. 화면과 접근성 문구, 코드 주석, 커밋과 풀 리퀘스트 문구, 이슈 문구, 릴리스 안내와 재사용할 문구도 포함한다. 이 자료는 한국어 부분에만 적용한다.
...
- 영어 용어를 정해진 한국어 한 단어로 자동 변환하지 않는다. `계약`, `경로`, `공개`, `좁히다`, `박다`는 닫힌 목록이 아니라 예시다. 해당 분야에서 확립된 용어이며 지칭 대상, 행동, 조건과 결과가 분명할 때만 유지한다. 그 밖의 경우 실제 규칙이나 행동을 적고, 근거로 뜻을 정할 수 없으면 `needs human input`으로 판정한다.
...
- 스킬이 사용할 수 있는 범용 서브에이전트 한 명에게 읽기 전용 의미 검토를 맡기게 한다. 서브에이전트가 없으면 스킬의 주 에이전트 대체 절차를 따른다. 사용자가 검토 결과를 요청했을 때는 독립 검토를 수행하지 못했다고 밝히고, 답변을 보내기 전 검토에는 이 내부 상태를 덧붙이지 않는다.
