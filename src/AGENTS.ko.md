...
- 저장소가 다르게 정하지 않았다면 커밋하거나 다른 사람과 공유하는 저장소 내용을 공개 결과물로 본다. 여기에는 README, 문서, 소스 코드와 주석, Git 커밋과 PR 문구, Issue 문구, 릴리스 안내, 사용자에게 보이거나 스크린리더가 읽는 문구가 포함된다.
- 공개 결과물의 각 문장은 확인된 저장소 근거, 승인된 결정 또는 이미 승인된 공개 문구를 토대로 작성한다. 사용자 프롬프트, 에이전트 지침, 내부 작업 설명, 작업 메모, 검토 기준, 평가표, 출력 형식과 작업 과정 설명을 공개 문구의 출처로 사용하지 않는다.
- 내부 문구를 공개 결과물에 인용하거나 그대로 옮기거나 일부 표현만 바꿔 쓰지 않는다. 해당 정보가 독자에게 필요하면 확인된 제품 동작, 사용 안내 또는 승인된 정책으로 새로 쓴다. 공개 가능한 근거가 없으면 문구에서 빼고 사용자에게 별도로 알린다.
- 내부 식별자, 비공개 문서 참조, 구현에서만 쓰는 이름, 관리자 메모, 미해결 결정, 검토 메모와 공개 전 확인 목록을 공개 결과물에 넣지 않는다. 해당 상태 자체가 독자에게 필요하고 사람이 문구를 승인한 경우가 아니면 `TODO`, `TBD`, `추후 결정` 같은 임시 문구를 넣지 않는다.
- 소유권, 지원 범위, 보안, 호환성, 개인정보 처리, 기여 조건 또는 라이선스가 결정되지 않았으면 추정해서 쓰지 않는다. 필요한 결정은 별도로 보고하고, 승인된 근거가 생긴 뒤 공개 결과물을 수정한다.
- Git 커밋 메시지는 실제 변경사항과 변경 이유를 토대로 작성한다. 사용자에게 보이거나 스크린리더가 읽는 문구는 사용자가 하려는 일, 요소의 목적과 알아야 할 상태를 설명한다.
- 공개 결과물 수정과 저장소 관리 작업을 분리한다. 사용자가 명시적으로 허용하지 않으면 이슈, 계획 항목, 정책 파일 또는 커뮤니티 파일을 만들지 않는다.

### 공개 전 검사

1. 커밋 메시지 초안을 포함해 변경한 공개 결과물을 모두 확인한다.
2. 추가한 각 문장과 문자열이 저장소 근거, 승인된 결정 또는 승인된 공개 문구에서 나온 것인지 확인한다.
...
6. 독자에게 전달될 최종 형태로 다시 읽는다. 프로젝트가 아니라 에이전트의 작업을 설명하는 내용이 남아 있으면 공개하거나 커밋하지 않는다.
...
- 커밋 제목, 본문과 푸터에도 공개 결과물 규칙을 적용한다. 저장소에서 실제로 바뀐 내용과 변경 이유를 설명하고, 사용자 프롬프트, 에이전트 지침, 내부 작업 문구, 검토 기준, 비공개 자료 또는 에이전트의 작업 과정은 적지 않는다.
