...
- 저장소가 다르게 정하지 않았다면 커밋하거나 다른 사람과 공유하는 저장소 내용은 작성자 외의 독자가 읽는 글로 다룬다. 여기에는 README, 문서, 소스 코드와 주석, Git 커밋과 PR 문구, Issue 문구, 릴리스 안내, 사용자에게 보이거나 스크린리더가 읽는 문구가 포함된다.
...
- 내부 문구를 독자에게 전달할 내용에 인용하거나 그대로 옮기거나 일부 표현만 바꿔 쓰지 않는다. 해당 정보가 독자에게 필요하면 확인된 제품 동작, 사용 안내 또는 승인된 정책으로 새로 쓴다. 독자에게 제시할 수 있는 근거가 없으면 문구에서 빼고 사용자에게 별도로 알린다.
- 내부 식별자, 비공개 문서 참조, 구현에서만 쓰는 이름, 관리자 메모, 미해결 결정, 검토 메모와 커밋하거나 공유하기 전 확인 목록을 독자에게 전달할 내용에 넣지 않는다. 해당 상태 자체가 독자에게 필요하고 사람이 문구를 승인한 경우가 아니면 `TODO`, `TBD`, `추후 결정` 같은 임시 문구를 넣지 않는다.
...
1. 커밋 메시지 초안을 포함해 변경한 글과 문구를 모두 확인한다.
...
6. 독자에게 전달될 최종 형태로 다시 읽는다. 프로젝트가 아니라 에이전트의 작업을 설명하는 내용이 남아 있으면 공유하거나 커밋하지 않는다.
...
- 문서 주석은 처음 보는 사람도 호출 방법과 제약을 이해할 수 있도록 작성한다. 함수의 경우 요약 설명, `@param`, `@returns`를 기본으로 포함하고, 부수 효과, 서버 렌더링 코드와 클라이언트 코드의 구분, 보안 제약, 예외 전달 방식처럼 호출자가 알아야 하는 내용은 `@remarks` 또는 `@throws`로 보강한다.
