...
- `본질적으로`, `다양한 측면에서`, `~라는 점에서 중요하다`, `효과적으로`, `원활하게`, `강력한`, `견고한`, `포괄적인`, `핵심적인`처럼 뜻을 보태지 않는 상투적 표현과 과장된 수식어를 쓰지 않는다.
- 소제목마다 같은 길이의 문단을 만들거나, 근거 없이 세 항목으로 나누거나, 같은 연결 표현을 되풀이하지 않는다. 내용에 맞춰 글의 짜임을 정한다.
- 영어를 낱말 단위로 옮기지 않는다. 원문이 가리키는 대상, 동작, 조건과 책임 주체를 확인한 뒤 한국어 문장을 새로 쓴다.
  - `~에 대해서`, `~에 의해서`, `~에 있어서`, `~와의`가 없어도 뜻이 같으면 조사나 문장 구조를 바꿔 간결하게 쓴다.
  - `~에 의해서`처럼 행동 주체를 감추는 피동문은 실제 행동 주체를 주어로 삼아 다시 쓴다.
  - `~하지 않으면 안 된다`는 `~해야 한다`로, `회의를 가지다`는 `회의하다`나 `회의를 열다`로 고친다.
  - 영어식 무생물 주어, 명사만 이어 붙인 문장, 불필요한 피동문, `사용자-facing` 같은 혼합 표기를 쓰지 않는다.
- 영어 기술 용어를 정해진 한국어 한 단어로 자동 변환하지 않는다. 문맥에 따라 실제 동작이나 조건을 드러낸다.
  - `boundary`는 `구분`, `적용 범위`, `책임이 바뀌는 지점`, `연결 지점` 중 실제 뜻에 맞는 표현을 쓴다.
  - `contract`는 `입출력 규칙`, `호출 조건`, `호환 조건`, `합의한 내용`처럼 무엇을 지켜야 하는지 밝힌다.
  - `narrow`는 `검토 대상을 정하다`, `후보를 줄이다`, `조건을 구체화하다`처럼 무엇이 어떻게 달라지는지 쓴다.
  - `ensure`, `support`, `address`는 각각 `보장하다`, `지원하다`, `다루다`로 고정하지 않고 `확인하다`, `허용하다`, `처리하다`, `해결하다`, `설명하다` 중 실제 동작을 쓴다.
  - `expose`, `capture`, `align`, `surface`는 `노출하다`, `포착하다`, `정렬하다`, `표면화하다`로 바로 옮기지 않고 `공개하다`, `기록하다`, `맞추다`, `드러내다`처럼 문맥에 맞게 쓴다.
  - 법률상 계약, 수학적 경계, 화면 정렬, 보안 정보 노출처럼 해당 분야에서 뜻이 정확하다면 원래 표현을 쓸 수 있다.
...
- 스킬이 사용할 수 있는 범용 서브에이전트 한 명에게 읽기 전용 의미 검토를 맡기게 한다. 서브에이전트가 없으면 스킬의 주 에이전트 대체 절차를 따르고 독립 검토를 수행하지 못했다고 밝힌다.
- 적용되는 항목마다 `pass`, `needs revision`, `needs human input`, `not applicable` 중 하나를 보고한다. 통과하지 못한 결과에는 위치와 이유를 함께 적는다.
