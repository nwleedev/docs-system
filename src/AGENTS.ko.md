...
- 일반 채팅, 진행 알림, 확인과 설명 답변을 작성할 때는 이 작업만을 이유로 `references/korean.md`를 불러오거나 `use-words-review`를 실행하지 않는다. 중간 메시지와 최종 답변은 아래 작성 규칙에 맞게 처음부터 쓴다.
...
- 영어 용어를 정해진 한국어 한 단어로 자동 변환하지 않는다. `계약`, `경로`, `공개`, `좁히다`, `박다`는 고정된 단어 목록이 아니라 예시다. 해당 분야에서 확립된 용어이며 지칭 대상, 행동, 조건과 결과가 분명할 때만 유지한다. 그 밖의 경우 실제 규칙이나 행동을 적고, 근거로 뜻을 정할 수 없으면 추측하지 말고 필요한 정보를 사용자에게 묻는다.
...
- AI가 만들거나 고친 한국어 글 또는 이름을 저장, 커밋, 게시하거나 대화 밖에서 공유하기 전에는 설치된 `use-words-review` 스킬의 `references/korean.md`를 읽고 해당 스킬로 검토한다. 화면과 접근성 문구, 코드 주석, 커밋과 풀 리퀘스트 문구, 이슈 문구와 릴리스 안내도 포함한다. 이 자료는 한국어 부분에만 적용한다.
- 채팅에서 작성한 문구는 사용자가 검토를 명시적으로 요청했거나 그 문구를 그대로 저장, 게시 또는 대화 밖에서 전달할 때만 검토 대상으로 삼는다. 일반 채팅 답변은 전송된다는 이유만으로 공유 결과물에 포함하지 않는다.
