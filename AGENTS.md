...
- 외부 Rust/WASM 라이브러리나 관련 도구를 반복해서 수정할 때에는 애플리케이션 저장소 안에 영구 복제본을 두지 않고 별도의 소스 저장소를 사용한다.
- 외부 소스 저장소의 위치는 `EXTERNAL_SOURCE_DIR` 환경 변수로 지정한다.
- 빌드 스크립트나 절차 문서는 `EXTERNAL_SOURCE_DIR` 값이 없으면 필요한 소스 위치를 지정하도록 안내한다.
- 별도 spike clone이 필요한 경우에는 기존 임시 파일 규칙에 따라 `temps/<scope>-<nonce>/` 아래에 만들고, 해당 clone을 제품 source of truth로 커밋하지 않는다.
- 장기 수정 이력은 외부 소스 저장소의 포크 브랜치와 커밋으로 관리하고, 애플리케이션 저장소에는 다시 빌드한 배포 파일과 연동 코드, 문서, 빌드 근거만 반영한다.
- 외부 도구를 다시 빌드해 붙일 때에는 원본 태그와 커밋, 포크 브랜치와 커밋 또는 패치 식별자, Rust/Node/wasm-pack 버전, Vite 기준 경로, PWA 제거와 캐시 정리 여부, 주요 파일의 체크섬을 기록한다.
- 외부 소스 전체를 애플리케이션 저장소의 `vendor/<external-project>`, `packages/<external-project>` 또는 하위 모듈로 들여오는 방식은 CI 재현 빌드나 감사 요구가 명시된 경우에만 별도 결정 기록을 남긴 뒤 검토한다.
